...
public:
	...
	Student(string num, string nam, string maj, string cit, string gra) {
		number = num;
		name = nam;
		major = maj;
		city = cit;
...
Student* arrayt(Student s[]) {//이름순재배열
	for (int i = 0; i < a; i++) {
		for (int k = 0; k < a; k++) {
			if (s[i].name < s[k].name) {
				Student a("a","a","a","a","a");
				a.name = s[i].name;
				a.number = s[i].number;
				a.city = s[i].city;
...
//프린트
void printarray(Student s[]) {//재배열후 프린트
	arrayt(s);
	for (int i = 0; i <= a; i++) {
...
void majorsoo(Major s[]) {
	cout <<"총 학과의 수는"<< countmajor - 1 << "개 : ";
	
	for (int i = 0; i < countmajor - 1; i++) {
		cout << majors[i].name << ",";
	}
	cout << endl;
...
int main() {
...

	{
		...
		student=arrayt(student);
		printarray(student);
		
	
		
		remove(student, "김상윤");
		search(student,"김상윤", 0, a-1);//이진탐색
